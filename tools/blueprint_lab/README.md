...
Byproduct recycle merge routes may reuse an existing belt only under a strict rule: the occupied entity must be the same transport-belt tier, have the same direction, and be a plain transport belt rather than a splitter or underground belt. This lets generated recycle corridors merge into already-routed input lanes without treating every overlap as a collision, while avoiding ambiguous splitter or underground semantics. A small fixture now proves that a recycle merge can reuse two existing westbound turbo belts and still pass `recycle_flow_audit`. On the real forced 3x2 pre-fanin sample, a diagnostic corridor probe showed why plain surface corridors were not enough: the four `scope=output-fanin` byproduct separators were blocked by existing input lanes, inserter pickup/output positions, or opposite-direction belts. The materializer therefore now tries a dedicated vertical underground crossing before falling back to surface merge, return routes, or finite buffers. The latest offline forced 3x2 pre-fanin sample still preserves the exact `2x turbo-transport-belt` external contract with zero connector collisions, and all four pre-fanin separators use `pre-fanin-recycle-underground-corridor-to-input-boundary` with `recycle_flow=pass` and no pre-fanin overflow belts. This is progress on recycle stability, not a solved strict blueprint: one later boundary-route separator still uses a finite overflow buffer, and both final output lanes remain `target-overloaded-after-pre-fanin-separation` at `4725/min > 3600/min`. The next strict step is lane-aware target-output compression after byproduct separation, followed by a real Factorio runtime proof.
