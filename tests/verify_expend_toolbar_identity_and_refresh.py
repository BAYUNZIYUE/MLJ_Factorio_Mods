...
def main() -> int:
    ...
    for path in SRC.rglob("*"):
        if path.is_file() and path.suffix in {".lua", ".json", ".cfg"}:
            ...
            assert_not_contains(path, "pipette")
    ...
    assert_contains(SRC / "panel.lua", "local function cursor_holds(player, slot)")
    assert_contains(SRC / "panel.lua", "if state.moving and not cursor_holds(player, state.moving.slot) then")
    ...
    assert_contains(SRC / "panel.lua", "elseif moving then")
    ...
    assert_contains(SRC / "panel.lua", "event.element.location = bar.place")
