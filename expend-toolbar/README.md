...
- Create multiple draggable custom bars with ten fixed numbered pages and item slots.
...
- The toolbar height follows the tallest page in that toolbar, so switching pages does not resize the window.
...
- Double-click a page number to rename that page.
...
- 创建多个可拖动自定义栏，每个自定义栏固定包含 10 个数字页面和物品槽。
...
- 每个数字页面都会保持这个横向槽位数。
- 工具栏高度会使用同一个工具栏里最高页面的高度，因此切换页面不会改变窗口大小。
