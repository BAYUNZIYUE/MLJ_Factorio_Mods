...
- Slot payloads must preserve special tools when possible: normal items use `name` plus `grade`, exported blueprint-like stacks use `LuaItemStack.export_stack()`, and cursor records keep `LuaRecord` metadata such as type, label, entity count, and book size. Prefer `player.cursor_stack` over `player.cursor_record` so real blueprint items are exported as stacks; only pure cursor records should use `LuaRecord.export_record()` during slot placement, with lightweight metadata and matching tool-item fallback if export or import fails.
