...
- Shared mod constants and names: `expend-toolbar/src/Toolbars.lua`
- Runtime event registration: `expend-toolbar/src/control/`
- GUI components: `expend-toolbar/src/src/gui/`
- Inventory and count refresh logic: `expend-toolbar/src/src/player/inventory/`
- Player/view selection logic: `expend-toolbar/src/src/player/Player.lua`
...
- Runtime code belongs under `src/control.lua`, `src/control/`, and `src/src/`; prototype and sprite registration belongs under `src/data*.lua` or `src/data/`; settings definitions belong under `src/settings*.lua` or `src/settings/`.
...
- Do not move runtime libraries out of `src/src/` as a cleanup-only change; the local `import()` helper expects that module prefix.
